...
function ProjectWorkbench() {
  ...
  const nextStep = (() => {
    ...
    if (isFinished) return { title: '下载 PPTX', desc: '已导出完成，点击上方「下载」保存文件。' }
    if (progress.pipeline_mode === 'auto') return { title: '生成已启动', desc: '一键生成已按八项配置锁定确认，正在推进中（如状态未变化请稍候刷新）。' }
    return { title: '完成八项确认', desc: '点击下方「打开 Confirm UI」完成受众/目标/风格等八项确认，确认后自动开始生成。' }
  })()
