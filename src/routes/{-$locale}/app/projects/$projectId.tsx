...
function ProjectWorkbench() {
  ...
  const showOutlineApproval = progress.status === 'spec_ready' && !!spec
  ...
  return (
    <AppShell user={user} isPro={isPro} active="dashboard" crumb={progress.name} paymentFailed={ent.paymentFailed}>
      ...
      <section className="mb-5 rounded-[14px] border border-border bg-card p-[18px]"><h2 className="mb-4 font-mono text-sm uppercase tracking-wide text-fg-3">Workflow</h2><div className="flex flex-wrap items-end gap-3"><label className="text-sm text-fg-2">Pages<Input value={pageCount} onChange={(event) => setPageCount(event.target.value)} className="mt-1 w-28" inputMode="numeric" /></label><button type="button" onClick={confirm} disabled={confirming} className="rounded-[7px] border border-border px-4 py-2 text-sm font-medium text-foreground disabled:opacity-50">{confirming ? 'Saving…' : 'Lock confirmations'}</button><button type="button" onClick={generate} disabled={generating} className="inline-flex items-center gap-2 rounded-[7px] bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50"><Play size={16} />{generating ? 'Starting…' : 'Start generation'}</button>{showOutlineApproval && <button type="button" onClick={approveOutline} disabled={approvingOutline} className="inline-flex items-center gap-2 rounded-[7px] border border-border px-4 py-2 text-sm font-medium text-foreground disabled:opacity-50"><CheckCircle2 size={16} />{approvingOutline ? 'Approving…' : 'Approve outline'}</button>}{showExportApproval && <button type="button" onClick={approveExport} disabled={approvingExport} className="inline-flex items-center gap-2 rounded-[7px] bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">{approvingExport ? 'Exporting…' : 'Approve export'}</button>}</div>{error && <p role="alert" className="mt-3 text-sm text-red-400">{error}</p>}</section>
