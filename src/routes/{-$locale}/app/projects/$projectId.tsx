...
import { pptmasterStatusLabel, PPTMASTER_RUNNING_STATUSES, PPTMASTER_REVIEWABLE_STATUSES, PPTMASTER_FAILED_STATUSES, PPTMASTER_PIPELINE_STAGES, pptmasterProgressPercent, pptmasterStageIndex, pptmasterStallSeconds } from '@/features/pptmaster/status'
...
function ProjectWorkbench() {
  ...
  return (
    <AppShell user={user} isPro={isPro} active="dashboard" crumb={progress.name} paymentFailed={ent.paymentFailed}>
      ...
      <section className="mb-5 rounded-[14px] border border-border bg-card p-[18px]"><h2 className="mb-4 font-mono text-sm uppercase tracking-wide text-fg-3">生成进度</h2>{(() => {
        const pct = pptmasterProgressPercent(progress.status, progress.svg_count, progress.expected_pages)
        const stageIdx = pptmasterStageIndex(progress.status)
        const stalled = pptmasterStallSeconds(progress)
        const stallThreshold = progress.executor_stall_limit_sec ?? 300
        const stalledShow = stalled > stallThreshold
        if (pct === null) return null
        return (
          <div className="mb-4">
            <div className="mb-1.5 flex items-center justify-between gap-2 text-xs">
              <span className="font-medium text-foreground">{pptmasterStatusLabel(progress.status)}</span>
              <span className={stalledShow ? 'font-medium text-amber-400' : 'text-fg-3'}>{stalledShow ? `⚠ ${Math.floor(stalled / 60)} 分钟没有新进展` : `${pct}%`}</span>
            </div>
            <div className="flex gap-1">
              {PPTMASTER_PIPELINE_STAGES.map((stage, i) => (
                <div key={stage.key} title={stage.label} className={`h-1.5 flex-1 rounded-full transition-colors ${stageIdx === null || i > stageIdx ? 'bg-border/70' : i === stageIdx ? (stalledShow ? 'bg-amber-400' : 'animate-pulse bg-primary') : 'bg-primary/80'}`} />
              ))}
            </div>
            <div className="mt-1 flex items-center justify-between text-[11px] text-fg-3">
              <span>{PPTMASTER_PIPELINE_STAGES.map((s) => s.label).join(' → ')}</span>
            </div>
            {progress.status === 'generating' && progress.expected_pages ? (
              <p className="mt-1 text-[11px] text-fg-3">已生成 {progress.svg_count ?? 0}/{progress.expected_pages} 页</p>
            ) : null}
            {progress.force_finalize_imminent && (
              <p className="mt-1 text-[11px] font-medium text-amber-400">页面已齐，正在等待收尾——系统即将自动完成，无需操作。</p>
            )}
            {stalledShow && (
              <p className="mt-1 text-[11px] font-medium text-amber-400">这个阶段较长时间没有新进展，系统会自动接管；也可以稍后刷新页面再看看。</p>
            )}
          </div>
        )
      ...
      {progress.pipeline_mode !== 'auto' && <section className="mb-5 rounded-[14px] border border-border bg-card p-[18px]"><h2 className="mb-3 font-mono text-sm uppercase tracking-wide text-fg-3">风格与内容确认</h2><p className="mb-4 text-sm text-fg-2">配置受众、目标、语言、风格、页数等八项内容，AI 会严格按照你的配置生成。点击「开始配置」打开配置页。</p><button type="button" onClick={openConfirmUi} disabled={openingConfirm || isGenerating} className="rounded-[7px] bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">{openingConfirm ? '正在打开…' : isGenerating ? '生成中，暂不可修改配置' : '开始配置'}</button>{isGenerating && <p className="mt-3 text-sm text-fg-2">当前正在生成页面（{pptmasterStatusLabel(progress.status)}），配置已锁定，等生成完成后才能修改。</p>}</section>}
